...
public class Handlers {
   ...
   public static class HandlerThread extends Thread {
      ...
      public HandlerThread(Cluster c, Session s, int i, HttpExchange he) {
         cluster = c;
         session = s;
         id = i;
      }
   ...
   public static class DbPostHandler implements HttpHandler {
      ...
      public void connect() {
         cluster = Cluster.builder().addContactPoint("reto4db").build();
         session = cluster.connect("http_request_log");
      }

      @Override
      public void handle(HttpExchange he) throws IOException {
         HandlerThread ht = new HandlerThread(cluster, session, ++id, he);
         ht.start();
      }
   }
}
